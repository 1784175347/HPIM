...
class SA:
    ...
    def edp(self, latency, energy):                 
        return latency * energy / (10**12)
    ...
    def HMSIM(self, tilenum_new, tile_type_new,  PE_num_new, xbar_size_new, mapping_new, tile_connection, topology, c):
        ...
        if len(area) == 0 or len(power) == 0 or len(latency) == 0:
            ...
            return area_floorplan, power[0], latency[0], energy[0]
    ...
    def HMSIM_SimConfig_self(self,filename):
        ...
        with open("SA_edp.txt", "a") as file:  
    ...
    def run_layer(self, net='vgg8', dataset='cifar10',tiletype='NVM',penum=1,xbarsize=1024,tileconnext=0,topology=0,c=2):
        ...
        device1 = Device(T=self.T, area=self.area, power=self.power, latency=self.latency, energy=self.energy, f=(self.func(self.area, self.power, self.latency, self.energy)), edp=self.edp(self.latency, self.energy), time = time.time()- start_time, tilenum = self.tilenum,tile_type = self.tile_type_layer, PE_num = self.PE_num_layer, xbar_size = self.xbar_size_layer, tile_connect = self.tile_connection, topology = self.topology, c = self.c)
        device1.write_to_file("SA_edp.txt")
    ...
    def run_tile(self, net, dataset,tile_type_layer, PE_num_layer, xbar_size_layer, layertilenum, tile_connection, topology, c):
        ...
        self.area, self.power, self.latency, self.energy = self.HMSIM(self.tilenum, self.tile_type,  self.PE_num, self.xbar_size, self.mapping, self.tile_connection, self.topology, self.c)
        device1 = Device(T=self.T, area=self.area, power=self.power, latency=self.latency, energy=self.energy, f=(self.func(self.area, self.power, self.latency, self.energy)), edp=self.edp(self.latency, self.energy), time = time.time()- start_time)
        device1.write_to_file("SA_edp.txt")
        self.history['f'].append(self.func(self.area, self.power, self.latency, self.energy))
        self.history['T'].append(self.T)
        stepnum = 0
...
class Device:
    def __init__(self, T, area, power, latency, energy, f, edp, time, tilenum = None,tile_type = None, PE_num = None, xbar_size = None, tile_connect = None, topology = None, c = None):
        self.T = T
        self.area = area
        self.power = power
    ...
    def write_to_file(self, filename):
        with open(filename, "a") as file:  
            ...
            file.write(f"edp={self.edp}ms*mJ\n")
...
def SA_run_edp():
    ...
    sa = SA(T0=100, Tf=100, alpha=0.99, k=1, a=area_flag, b=power_flag, d=latency_flag, e=energy_flag, area_des=area_des, power_des=power_des, latency_des=latency_des, energy_des=energy_des,mix_mode='4')
    tiletype = 'SRAM'
    penum = 1
    xbarsize = 512
    tile_connection = 3
    ...
    sa.HMSIM_SimConfig_self(f'mix_tileinfo_edp_{dataset}_{network}.ini')
