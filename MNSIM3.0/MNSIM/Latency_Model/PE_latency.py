...
class PE_latency_analysis():
    def __init__(self, SimConfig_path, read_row=0, read_column=0, indata=0, rdata=0, inprecision = 8, default_buf_size = 16,device_type='none',xbar_size=[0,0],mix_mode=1):
        ...
        if (mix_mode==2 or mix_mode==4) and xbar_size[0]!=0:
            self.PE = ProcessElement(SimConfig_path,device_type=device_type,xbar_size=xbar_size,ADC_num=math.ceil(xbar_size[0]/8),DAC_num=math.ceil(xbar_size[0]/8))
        else:
            self.PE = ProcessElement(SimConfig_path,device_type=device_type,xbar_size=xbar_size)
        ...
        self.PE.calculate_xbar_read_latency(device_type=device_type)
