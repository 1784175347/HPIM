...
class crossbar(device):
	def __init__(self, SimConfig_path,device_type='none',xbar_size=[0,0]):
		device.__init__(self,SimConfig_path,device_type=device_type)
		...
		#linqiushi modified:
		if xbar_size!=[0,0]:
			self.xbar_size[0]=xbar_size[0]
			self.xbar_size[1]=xbar_size[1]
			self.subarray_size=self.xbar_size[1]
		...
		# mix_type is prior to PIM_type and device_type
		if device_type=='NVM':
	...
	def calculate_xbar_read_latency(self,device_type=None):
		# unit: ns
		...
		self.xbar_read_latency = self.device_read_latency_NVM + wire_latency
		if self.PIM_type_pe==0:
			self.xbar_read_latency = self.device_read_latency_NVM + wire_latency
		elif self.PIM_type_pe==1:
			self.xbar_read_latency = self.device_read_latency_SRAM + wire_latency
