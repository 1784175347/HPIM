...
class ProcessElement(crossbar, DAC, ADC):
    #linqiushi modified
    #add the mix_type into class ProcsssElement.init()
	def __init__(self, SimConfig_path,device_type='none',xbar_size=[0,0],ADC_num=0,DAC_num=0):
		crossbar.__init__(self, SimConfig_path,device_type=device_type,xbar_size=xbar_size)
		DAC.__init__(self, SimConfig_path,device_type=device_type)
		ADC.__init__(self, SimConfig_path,device_type=device_type)
		PE_config = cp.ConfigParser()
		PE_config.read(SimConfig_path, encoding='UTF-8')
		# mix_type has priority over the self.PIM_type_pe,'none' means no mixing
		...
		if device_type=='NVM':
			...
			self.PIM_type_pe=1
		#the init of mixing is completed
		#linqiushi above
		self.sub_position = 0
		...
		if ADC_num==0:
			...
			self.calculate_ADC_num(ADC_num=ADC_num)
		if DAC_num==0:
			self.calculate_DAC_num()
		else:
			self.calculate_DAC_num(DAC_num=DAC_num)
	...
	def calculate_ADC_num(self,ADC_num=0):
		...
		if ADC_num==0:
			...
			self.PE_ADC_num=ADC_num
			self.PE_group_ADC_num=math.ceil(ADC_num*self.subarray_num/self.group_num)	
			self.output_mux=16
	...
	def calculate_DAC_num(self,DAC_num=0):
		self.calculate_xbar_area()
		...
		if DAC_num==0:
			...
			self.PE_DAC_num=DAC_num
			self.PE_group_DAC_num=math.ceil(DAC_num*self.subarray_num/self.group_num)
	...
	def calculate_PE_area(self, SimConfig_path=None, default_inbuf_size = 16,mix_mode=1,ADC_num_mix=0,DAC_num_mix=0):
		# unit: um^2
		...
		elif (mix_mode==2 or mix_mode==4)and ADC_num_mix!=0 and DAC_num_mix!=0:
			self.PE_ADC_area = self.ADC_area*ADC_num_mix
			self.PE_DAC_area = self.DAC_area*DAC_num_mix
	...
	def calculate_PE_area_part(self, SimConfig_path=None, default_inbuf_size = 16,mix_mode=1,ADC_num_mix=0,DAC_num_mix=0):
		# unit: um^2
		self.calculate_xbar_area()
		self.calculate_demux_area()
		self.calculate_mux_area()
	...
	def calculate_PE_write_power_fast(self,max_column=0,max_row=0,max_group=0):
		self.PE_write_power=0
		self.PE_xbar_write_power=0
		self.xbar_write_config(write_row=max_row,write_column=max_column)
	...
	def calculate_PE_read_power_fast(self, max_column=0, max_row=0, max_group=0, SimConfig_path=None, default_inbuf_size = 16, mix_mode=1,
		...
		if ADC_num_mix!=0 and DAC_num_mix!=0:
			self.PE_DAC_read_power = DAC_num_mix * self.DAC_power
		else:
		...
		if self.PIM_type_pe == 0: # analog pim
			if ADC_num_mix!=0 and DAC_num_mix!=0:
				self.PE_ADC_read_power=ADC_num_mix*self.ADC_power
			else:
			...
			if ADC_num_mix!=0 and DAC_num_mix!=0:
				self.PE_ADC_read_power=ADC_num_mix*self.ADC_power*self.subarray_num
			else:
	...
	def calculate_PE_energy_efficiency(self, SimConfig_path=None):
		...
		self.calculate_xbar_read_latency(self.device_type)
