...
class device(object):
	def __init__(self, SimConfig_path,device_type='none'):
		...
		self.device_read_latency_NVM = float(device_config.get('Device level', 'Read_Latency_NVM'))
		self.device_read_latency_SRAM = float(device_config.get('Device level', 'Read_Latency_SRAM'))
		self.device_write_latency = float(device_config.get('Device level', 'Write_Latency'))
		#mix_type is prior to device_type
		if device_type=="NVM":
			self.device_type="NVM"
			self.device_area=float(device_config.get('Mixmode2/4 Configuration', 'NVM_device_area'))
		elif device_type=="SRAM":
			self.device_type="SRAM"
			self.device_area=float(device_config.get('Mixmode2/4 Configuration', 'SRAM_device_area'))
		if self.device_type == "NVM":
			...
			self.device_read_power = self.device_read_energy/self.device_read_latency_SRAM
